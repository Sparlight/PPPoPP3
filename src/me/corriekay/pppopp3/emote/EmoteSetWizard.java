...
public class EmoteSetWizard{
	...
	private void next(Player player){
		switch (stage){
			case 0:
				player.sendMessage(ChatColor.DARK_GRAY + "===================");
				player.sendMessage(gray + "Time to set your emotes Name! The name of your emote is what players type in when they use it. /mee <emoteName> <target>");
				break;
			case 1:
				player.sendMessage(ChatColor.DARK_GRAY + "===================");
				player.sendMessage(gray + "Time to set your sender text! The sender text is the message whoever is using your emote will see! \"<r>\" and \"<s>\" will be replaced automatically when used with the receiver and senders names, respectively.");
				break;
			case 2:
				player.sendMessage(ChatColor.DARK_GRAY + "===================");
				player.sendMessage(gray + "Time to set your receiver text! The receiver text is the message that the target will recieve! \"<r>\" and \"<s>\" will be replaced automatically when used with the receiver and senders names, respectively.");
				break;
			case 3:
				player.sendMessage(ChatColor.DARK_GRAY + "===================");
				player.sendMessage(gray + "Time to set the server message! The server text is the message that the rest of the server, besides you and the target, will see! \"<r>\" and \"<s>\" will be replaced automatically when used with the receiver and senders names, respectively.");
				break;
			case 4:
				player.sendMessage(ChatColor.DARK_GRAY + "===================");
				player.sendMessage(gray + "Time to set your emotes private status! Type either true, or false. If your emote is private, only you can use it!");
				break;
			case 5:
				player.sendMessage(ChatColor.DARK_GRAY + "===================");
				player.sendMessage(gray + "Time to set your emotes silent status! Type either true, or false. IF your emote is silent, only the sender and receiever may see it, but not the rest of the server!");
				break;
		}
	}
