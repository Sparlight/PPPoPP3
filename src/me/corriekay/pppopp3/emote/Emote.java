	...
	protected void useEmote(Player sender, Player reciever){
		...
		if(!isSilent) {
			for(Player player : players) {
				player.sendMessage(format(server, sender.getDisplayName(), reciever.getDisplayName()));
			}
		}
		Bukkit.getConsoleSender().sendMessage(format(server, sender.getDisplayName(), reciever.getDisplayName()));
		RemotePonyAdmin.rpa.message(ChatColor.stripColor(format(server, sender.getDisplayName(), reciever.getDisplayName())));
